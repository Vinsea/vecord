...
const WINDOW_ICON_FILENAME =
	process.platform === "darwin" ? "vecordmac-512.png" : "vecord-512.png";
...
function getEditorWindowQuery(): Record<string, string> {
	...
	const devInput = process.env.RECORDLY_DEV_OPEN_RECORDING_INPUT;
	if (devInput && !devInput.endsWith(".vecord") && !devInput.endsWith(".recordly") && !devInput.endsWith(".openscreen")) {
		query.devOpenInput = devInput;
	}
