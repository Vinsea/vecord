...
app.whenReady().then(async () => {
	...
	if (IS_SMOKE_EXPORT || process.env.RECORDLY_DEV_OPEN_RECORDING_INPUT) {
		...
		if (IS_SMOKE_EXPORT) {
			...
			console.log(`[dev-open-recording] Starting editor for ${devInput}`);
			if (devInput.endsWith(".vecord") || devInput.endsWith(".recordly") || devInput.endsWith(".openscreen")) {
				await openRecordlyFile(devInput);
				return;
			}
		}
		createEditorWindowWrapper();
